...
from .calibration import DetectorConfig, calibrate, save_config, load_config, validate_calibration
...
def show_recalibration_dialog(
    ...
    tk.Label(
        dialog,
        text="Recalibrate Switch Detection",
    ...
    tk.Label(
        settings_frame,
        text=f"Debounce: {current_config.debounce_ms} ms",
        anchor=tk.W
    ).pack(fill=tk.X, padx=10)
        ...
        def on_calibration_complete(new_config: DetectorConfig):
            ...
            try:
                ...
                messagebox.showerror(
                    "Error",
                    f"Failed to save calibration: {exc}"
                )
    ...
    def try_alternative_device():
        ...
        working_device, error, mode = manager.find_working_device(
            None,  # Try any device
            current_config.samplerate,
            current_config.blocksize
        )
        ...
        if working_device is not None:
            ...
            messagebox.showerror(
                "No Devices Found",
                f"No working audio devices found: {error}"
            )
    ...
    tk.Button(
        ...
        command=start_auto_calibration,
        width=15
    ).pack(side=tk.LEFT, padx=5)
    ...
    tk.Button(
        ...
        command=try_alternative_device
    ).pack(side=tk.LEFT, padx=5)
