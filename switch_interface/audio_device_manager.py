...
__all__ = ["AudioDeviceManager", "get_available_devices", "test_device", "find_working_device", 
           "AudioDeviceMode", "AudioDeviceError"]


class AudioDeviceError(Exception):
    """Exception raised for audio device errors with recovery information."""
    
    def __init__(self, message: str, device_id: Optional[int | str] = None, 
                 error_type: str = "general", recovery_hint: Optional[str] = None):
        self.device_id = device_id
        self.error_type = error_type  # "access", "format", "hardware", "not_found", "general"
...
class AudioDeviceManager:
    ...
    def __init__(self):
        ...
        self.device_test_cache: Dict[str, Tuple[bool, float]] = {}  # device_id -> (success, timestamp)
        self.cache_timeout = 30.0  # Cache test results for 30 seconds
        self.fallback_attempts: Dict[str, int] = {}  # Track fallback attempts per device
    ...
    def test_device(
        ...
        try:
            ...
            logger.debug(f"Device test failed for {device} in {mode} mode: {error_msg} (type: {error_type})")
    ...
    def _categorize_audio_error(self, error_msg: str) -> str:
        ...
        if "access denied" in error_msg or "permission" in error_msg or "being used" in error_msg:
            return "access"
        elif "format" in error_msg or "sample rate" in error_msg or "channels" in error_msg:
            return "format"
    ...
    def find_working_device(
        ...
        if preferred_device is not None:
            for mode in mode_fallbacks[preferred_mode]:
                logger.debug(f"Testing preferred device {preferred_device} in {mode} mode")
                success, error = self.test_device(preferred_device, samplerate, blocksize, mode=mode)
                
                if success:
                    logger.info(f"Found working audio device: {preferred_device} in {mode} mode")
        ...
        if (self.last_working_device is not None and 
            self.last_working_device != preferred_device and
            self.last_working_device in devices_to_try):
            
            # Try last working mode first, then fallbacks
            modes_to_try = [self.last_working_mode] + [m for m in ["auto", "shared", "exclusive"] 
                                                      if m != self.last_working_mode]
            
            for mode in modes_to_try:
                logger.debug(f"Testing last working device {self.last_working_device} in {mode} mode")
                success, error = self.test_device(self.last_working_device, samplerate, blocksize, mode=mode)
                
                if success:
                    logger.info(f"Found working audio device: {self.last_working_device} in {mode} mode")
                    self.last_working_mode = mode
        ...
        for device in devices_to_try:
            ...
            for mode in ["auto", "shared", "exclusive"]:
    ...
    def get_device_fallback_chain(
        ...
        # Last working device
        if self.last_working_device is not None and self.last_working_device != preferred_device:
...
def find_working_device(
    ...
    """
    manager = AudioDeviceManager()
    return manager.find_working_device(preferred_device, samplerate, blocksize, preferred_mode)