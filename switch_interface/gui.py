...
class FirstRunWizard(tk.Toplevel):
    ...
    def __init__(self, master: tk.Misc | None = None):
        ...
        self.retry_btn: tk.Button | None = None
        self.help_frame: tk.Frame | None = None
    ...
    def _step_device(self) -> tk.Frame:
        frame = tk.Frame(self)
        tk.Label(
            frame, text="Select audio device", font=("TkDefaultFont", 10, "bold")
        ).pack(pady=(0, 5))
        ...
        if not devices:
            ...
            help_btn = tk.Button(
                button_frame, text="Get Help", command=self._show_audio_help
            )
    ...
    def _start_calibration(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                if "No Default Input Device" in str(exc) or "Invalid device" in str(
                    exc
                ):
        ...
        if self.calib_data is not None:
            ...
            if self.help_frame is None:
                self.help_frame = tk.Frame(
                    self.steps[1], relief=tk.RIDGE, borderwidth=1
                )
                ...
                tips_text = tk.Text(
                    self.help_frame, height=4, wrap=tk.WORD, font=("TkDefaultFont", 8)
                )
    ...
    def _skip_calibration(self) -> None:
        ...
        self.status_var.set(
            "✓ Using default settings - your switch will work with most common setups"
        )
        self.next_btn.config(state=tk.NORMAL)
...
If you continue to have problems, you can skip calibration and use default settings. The application will still work with most switch setups."""
    ...
    def _step_calibration(self) -> tk.Frame:
        frame = tk.Frame(self)
        tk.Label(
            frame, text="Switch Calibration", font=("TkDefaultFont", 10, "bold")
        ).pack(pady=(0, 5))
        ...
        tk.Label(
            info_frame, text="Your Options:", font=("TkDefaultFont", 9, "bold")
        ).pack(pady=(5, 2))
        ...
        status_label = tk.Label(
            frame, textvariable=self.status_var, wraplength=350, justify=tk.CENTER
        )
    ...
    def _step_scan_speed(self) -> tk.Frame:
        frame = tk.Frame(self)
        tk.Label(
            frame, text="Scanning Speed Preferences", font=("TkDefaultFont", 10, "bold")
        ).pack(pady=(0, 5))
        ...
        tk.Label(
            tips_frame,
            text="💡 Tips for choosing speed:",
            font=("TkDefaultFont", 9, "bold"),
        ).pack(pady=(5, 2))

        tips_text = tk.Text(
            tips_frame, height=3, wrap=tk.WORD, font=("TkDefaultFont", 8)
        )
        tips_text.pack(fill=tk.X, padx=10, pady=5)
        tips_text.insert(
            tk.END,
