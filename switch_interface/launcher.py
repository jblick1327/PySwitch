...
class EnhancedLauncher:
    ...
    def create_ui(self) -> None:
        ...
        # Dwell time setting
        self.dwell_var = tk.DoubleVar(
            master=self.root, value=settings.get_scan_interval(cfg)
        )
        ...
        tk.Checkbutton(
            self.root, text="Row/column scanning", variable=self.rowcol_var
        ).pack(padx=10, pady=5)
        ...
        self.safe_mode_button = tk.Button(
            button_frame, text="Safe Mode", command=self._start_safe_mode
        )
    ...
    def _calibrate(self) -> None:
        ...
        assert self.root is not None, "UI must be created before calibration"
        try:
            ...
            self._update_status(
                "Calibration failed. You can skip it or try again.", "red"
            )

    def _start(self) -> None:
        """Start the main application with enhanced error handling."""
        assert self.root is not None, "UI must be created before starting"
        assert self.layout_var is not None, "Layout var must be initialized"
        assert self.dwell_var is not None, "Dwell var must be initialized"
        assert self.rowcol_var is not None, "Rowcol var must be initialized"
        try:
            ...
            self._update_status(
                "Startup failed. Try the suggestions above or use recovery options.",
                "red",
            )
    ...
    def _start_safe_mode(self) -> None:
        ...
        assert self.root is not None, "UI must be created before starting safe mode"
        try:
            ...
            self._show_error_dialog(error_info)
            self._update_status(
                "Safe mode also failed. Check hardware connections.", "red"
            )
    ...
    def _show_error_dialog(self, error_info: dict) -> None:
        ...
        if error_info["severity"] in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]:
            ...
            def enable_safe_mode_and_close():
                dialog.destroy()
                self._enable_safe_mode()

            tk.Button(
                button_frame, text="Try Safe Mode", command=enable_safe_mode_and_close
            ).pack(side=tk.RIGHT, padx=(5, 0))

    def _show_error_buttons(self, error_info: dict) -> None:
        """Show retry and safe mode buttons after an error."""
        assert self.retry_button is not None, "Retry button must be initialized"
        assert self.safe_mode_button is not None, "Safe mode button must be initialized"
    ...
    def _enable_safe_mode(self) -> None:
        ...
        self._update_status(
            "Safe mode enabled. Click 'Retry' to start with minimal functionality.",
            "orange",
        )
